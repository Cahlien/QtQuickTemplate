...
function(configure_release_distributables target)
    if (APPLE AND NOT IOS)
        # DMG pipeline: build → MacDeployQt → DMG → NotarizeMacOS → VerifyMacOSPackage
        ...
        # App Store pipeline: build → MacAppStoreArchive → MacExportPkg → VerifyMacPkg → MacUploadASC
        if (NOT TARGET ReleaseDistributableMacOSAppStore)
            set(_macos_appstore_dep "")
            set(_macos_appstore_comment "")
            if (TARGET MacUploadASC)
                set(_macos_appstore_dep MacUploadASC)
                set(_macos_appstore_comment "Building, archiving, exporting, verifying, and uploading macOS PKG to App Store Connect")
            elseif (TARGET VerifyMacPkg)
                set(_macos_appstore_dep VerifyMacPkg)
                set(_macos_appstore_comment "Building, archiving, and verifying macOS App Store PKG artifacts")
            elseif (TARGET MacExportPkg)
                set(_macos_appstore_dep MacExportPkg)
                set(_macos_appstore_comment "Building and exporting macOS App Store PKG artifacts")
            elseif (TARGET MacAppStoreArchive)
                set(_macos_appstore_dep MacAppStoreArchive)
                set(_macos_appstore_comment "Building macOS App Store archive artifacts")
            endif ()

            if (_macos_appstore_dep)
                add_custom_target(ReleaseDistributableMacOSAppStore
                    DEPENDS ${_macos_appstore_dep}
                    COMMENT "${_macos_appstore_comment}"
                )
                message(STATUS "ReleaseDistributableMacOSAppStore target configured -> cmake --build . --target ReleaseDistributableMacOSAppStore")
            endif ()
        endif ()
    endif ()
    ...
    if (TARGET ReleaseDistributableMacOSAppStore)
        add_dependencies(ReleaseDistributable ReleaseDistributableMacOSAppStore)
    endif ()
